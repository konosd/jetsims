...
def loadOne(cohesins, occupied, args): 
    ...
    if args['Probabilistic_Loading']:
        while True:
            # First toss a coin to see whether we have site loading or not
            loading_on_loading_site = np.random.uniform()>0.2
            if loading_on_loading_site:
                ...
                a = np.random.randint(args['M'])*args['N1']+np.random.randint(args['bLS'][0], args['bLS'][1])
                if (occupied[a] == 0) and (occupied[a+1] == 0):
                    occupied[a] = 1
                    occupied[a+1] = 1 
                    cohesins.append(cohesin(leg(a), leg(a+1)))
